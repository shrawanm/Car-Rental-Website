...
error_reporting(0);
$bookingError = false; // Flag for booking conflict
...
if(isset($_POST['submit']))
{
    ...
    if ($query1->rowCount() == 0)
    {
        ...
        if ($lastInsertId)
        {
            echo "<script>alert('Booking successful. Redirecting to payment...');</script>";
            echo "<script type='text/javascript'> document.location = 'payment_method.php?bookingid=$lastInsertId'; </script>";
            exit;
        }
        else
        {
    ...
    {
        $bookingError = true; // Set flag for display error
    }
}
?>
<!DOCTYPE HTML>
  ...
  <div class="sidebar_widget">
    ...
    <?php if ($bookingError): ?>
      <div class="alert alert-danger" style="margin-bottom:15px;">
        Sorry! This car is already booked for the selected dates. Please choose different dates.
      </div>
    <?php endif; ?>

    <form method="post" id="bookingForm" onsubmit="return validateDates()">
      <div class="form-group">
        <label>From Date:</label>
        <input type="date" class="form-control" name="fromdate" id="fromdate" required min="<?php echo date('Y-m-d'); ?>">
      </div>
      <div class="form-group">
        <label>To Date:</label>
        <input type="date" class="form-control" name="todate" id="todate" required min="<?php echo date('Y-m-d'); ?>">
      </div>
      <div class="form-group">
...
<script>
// Function to validate that the 'From' date is not after the 'To' date
function validateDates() {
    // Get the value of the 'fromdate' input and convert it to a Date object
    const fromDate = new Date(document.getElementById('fromdate').value);
    
    // Get the value of the 'todate' input and convert it to a Date object
